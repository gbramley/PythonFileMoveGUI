...
import os, time, datetime, shutil, sqlite3
...
class CopyFileGUI:
    def __init__(self, master):
        ...
        ### Frame
        master.geometry('500x375')  # (WidthxHeight)
        ...
        self.tofolderLabel.grid(row=4, column=0, padx=5, sticky='sw')

        self.tofolderButton = ttk.Button(master, text='Browse Destination Folder...',
                                         command=lambda: self.selectDir(self.tofolderEntry))
        self.tofolderButton.grid(row=4, column=1, sticky='e', padx=5)

        self.tofolderEntry = Entry(master, width=80)
        self.tofolderEntry.grid(row=5, column=0, columnspan=2, padx=5)

        self.copyfilesButton = ttk.Button(master, text='Copy Recent Files!')
        self.copyfilesButton.grid(row=6, column=0, columnspan=2, pady=10)
        ...
        self.lastCopyLabel.grid(row=7, column=0, columnspan=2)
    ...
    def copyTime_entry(self):
        self.conn = sqlite3.connect('timestamps.db')
        self.c = self.conn.cursor()
        self.c.execute("INSERT INTO copyTimes (epochTime, datestamp) VALUES (?, ?)",
                       (self.now, str(time.strftime('%Y-%m-%d %H:%M:%S'))))
        self.conn.commit()
        self.c.close()
        self.conn.close()
